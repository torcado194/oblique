...
function bezierDeriv(p1, p2, p3, p4, t){
	return {
  	x: 3*((1-t)**2)*(p2.x-p1.x) + 6*((1-t)*t)*(p3.x-p2.x) + 3*(t**2)*(p4.x-p3.x),
    y: 3*((1-t)**2)*(p2.y-p1.y) + 6*((1-t)*t)*(p3.y-p2.y) + 3*(t**2)*(p4.y-p3.y)
  };
}

function getNetworkCenter(network:VectorNetwork){
  let acc;
